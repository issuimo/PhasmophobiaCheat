...
auto APIENTRY DllMain(HMODULE hModule, const DWORD ul_reason_for_call, LPVOID lpReserved) -> BOOL {
	switch (ul_reason_for_call) {
		case DLL_PROCESS_ATTACH:
			...
			if (GetModuleHandle(L"Phasmophobia.exe")) {
				std::thread([hModule] {
					...
					dx_hook::Hk11::Build([hModule] {
						...
						if (!init_space::Info::tipsShow) {
							if (ImGui::Begin(reinterpret_cast<const char*>(u8"Phasmophobia Cheat By 遂沫(github@issuimo)"), &init_space::Info::show, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings)) {
...
extern "C" _declspec(dllexport) auto Inject() -> void {
	if (const auto pid = GetIdByName(L"Phasmophobia.exe")) {
		EnumWindows(EnumFunc, pid);
		if (auto lParam = SetWindowsHookExW(WH_GETMESSAGE, HookCallBack, init_space::Info::hModule, idThread)) PostThreadMessageW(idThread, WM_NULL, 0, 0);
	}
}
