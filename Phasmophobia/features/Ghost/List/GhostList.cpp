...
void GhostList::Render() {
    if (ImGui::BeginTable("GhostList", 8,
		...
		if (!Player::players.empty())
			if (Ghost::ghost)
				if (Ghost::ghost->ghostInfo && Ghost::ghost->ghostInfo->room && Ghost::ghost->ghostInfo->room->name && Ghost::ghost->ghostInfo->proof && Player::players[0]->journalController, Player::players[0]->journalController->textMeshProUgui) {
					ImGui::TableNextRow();
